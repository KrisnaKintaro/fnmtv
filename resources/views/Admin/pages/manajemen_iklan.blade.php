...
@section('css')
    <style>
        /* FIX RESPONSIVE: Scroll horizontal tabel */
        .partnership-wrap {
            width: 100%;
            overflow-x: auto;
...
@section('konten')
    <div class="content-body" style="padding: 20px 24px;">
        <div class="section-header"
            ...
            <button onclick="ModalManager.open('modalContentModule')" class="btn-primary"
                style="padding: 10px 20px; border-radius: 8px; background: var(--red); color: white; border: none; font-weight: 600; cursor: pointer;">
                <i class="fas fa-plus" style="margin-right: 8px;"></i> Tambah Iklan
            </button>
        </div>

        <div class="card"
            style="background: white; border-radius: 12px; border: 1px solid var(--border); overflow: hidden;">
            <div class="partnership-wrap">
                <table class="partnership-table" id="contentModuleTable">
                    ...
                    <tbody id="contentModuleList">
                        <tr>
                            <td colspan="5" style="text-align: center; padding: 40px; color: var(--muted);">⏳ Memuat data...</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            <div id="emptySearch" style="display:none; text-align:center; padding:40px; color:var(--muted);">
    ...
    <div id="modalContentModule" class="modal-backdrop">
        <div class="modal special-announcement-modal">
            ...
            <form id="formContentModule" enctype="multipart/form-data">
                ...
                <div style="margin-bottom: 16px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">Tanggal Mulai Tayang <span
                ...
                <div style="margin-bottom: 16px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">Tanggal Berhenti Tayang <span
                            style="color: var(--red);">*</span></label>
                ...
                <div style="margin-bottom: 16px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">Unggah Gambar <span
                            id="gambarRequired" style="color: var(--red);">*</span></label>
                    <input type="file" id="moduleImageInput" name="gambar" accept="image/png, image/jpeg, image/jpg"
                        style="display:none;">
                ...
                <div style="display: flex; gap: 12px; margin-top: 24px;">
                    <button type="button" onclick="ModalManager.close('modalContentModule'); resetFormModule();"
                        style="flex: 1; padding: 12px; border-radius: 8px; border: 1px solid var(--border); background: white;">Batal</button>
                    <button type="submit" id="btnSimpan"
                        style="flex: 1; padding: 12px; border-radius: 8px; background: var(--red); color: white; border: none; font-weight: 600;">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal-backdrop" id="modalRemoveConfirm" style="display:none;">
        <div class="modal" style="max-width:380px; text-align:center; padding:30px;">
            ...
            <p style="font-size:13px; color:var(--muted); margin-bottom:24px;">Konten yang dihapus tidak bisa dikembalikan.
            </p>
            <div style="display:flex; gap:10px;">
                <button class="btn btn-outline" style="flex:1; justify-content:center;"
                    onclick="ModalManager.close('modalRemoveConfirm')">Batal</button>
                <button class="btn btn-red" style="flex:1; justify-content:center;" onclick="executeRemoveEntry()">Ya,
                    Hapus</button>
            </div>
        </div>
    </div>
...
@section('js')
    <script>
        ...
        function resetFormModule() {
            ...
            $('#gambarRequired').show(); // Bintang merah wajib gambar
            editingEntryId = null;
        }
        ...
        $(document).ready(function() {
            ...
            // 🔥 SINKRONISASI SEARCH BAR GLOBAL (NAVBAR) KE TABEL LOKAL 🔥
            const searchInputs = document.querySelectorAll('#searchInput, .mobile-search-input');
            searchInputs.forEach(input => {
                if (input) {
                    input.placeholder = 'Cari judul iklan...';
                    input.addEventListener('keyup', function() {
                        const keyword = this.value.toLowerCase();
                        let found = false;

                        $('#contentModuleList tr').each(function() {
                            const textInfo = $(this).text().toLowerCase();
                            if (textInfo.indexOf(keyword) > -1) {
                                $(this).show();
                                found = true;
                            } else {
                                $(this).hide();
            ...
            $('#formContentModule').on('submit', function(e) {
                ...
                var method = 'POST'; // Backend laravel lu nrima POST (atau _method PUT)
            ...
            // LOGIKA DRAG & DROP GAMBAR LU (TETAP UTUH 100%)
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (!data || data.length === 0) {
                        html = '<tr><td colspan="5" style="padding: 40px; text-align: center; color: var(--muted);">Belum ada konten iklan. Tambahkan konten baru untuk memulai.</td></tr>';
                    } else {
                        $.each(data, function(i, item) {
                            ...
                            if (item.link_tujuan) {
                                infoHtml += '<div style="font-size: 12px; margin-top: 4px;">' +
                                    '<a href="' + item.link_tujuan + '" target="_blank" style="color: var(--blue); text-decoration: none;">' +
                                    item.link_tujuan + '</a></div>';
                            }
                            if (item.tanggal_mulai) {
                                infoHtml += '<div style="font-size: 12px; color: var(--muted); margin-top: 4px;">Mulai: ' +
                                    formatTanggal(item.tanggal_mulai) + '</div>';
                            }
                            if (item.tanggal_selesai) {
                                infoHtml += '<div style="font-size: 12px; color: var(--muted); margin-top: 4px;">Habis: ' +
                                    formatTanggal(item.tanggal_selesai) + '</div>';
                            }
                            ...
                            html += '<img src="/storage/' + item.gambar + '" loading="lazy" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\';">';
                            ...
                            html += '<td style="padding: 14px 16px;">';
                            html += '<button onclick="editEntry(' + item.id + ')" class="remove-entry" title="Edit" style="color: var(--blue); margin-right: 8px;">✏️</button>';
        ...
        function editEntry(id) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (item) {
                        editingEntryId = id;
                        $('#modalTitle').text('Edit Iklan / Promo');
                        ...
                        $('#gambarRequired').hide(); // Hide bintang merah karena pas edit gambar gak wajib
                ...
                error: function(err) {
                    Toast.show('error', 'Gagal memuat data edit');
                    console.error('Error:', err);
                }
            });
        }
