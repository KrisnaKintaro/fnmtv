...
<style>
    ...
    @media screen and (max-width: 768px) {
        .profile-grid {
            grid-template-columns: 1fr !important; /* Paksa numpuk ke bawah di HP */
            gap: 16px;
        }
...
<script>
    $(document).ready(function() {
        ...
        $('#formProfilEditor').on('submit', function(e) {
            ...
            const payload = {
                _method: 'PUT', // Method spoofing buat Laravel
            ...
            $.ajax({
                ...
                type: 'POST', // Tetap dikirim via POST karena dibajak method spoofing di atas
                data: payload,
                success: function(res) {
                    Toast.show('success', res.message || 'Profil berhasil diperbarui!');
                    btn.html(originalText).prop('disabled', false);

                    // Kosongkan semua field password setelah berhasil simpan
                    $('#profCurrentPass, #profPassword, #profPasswordConfirm').val('');

                    // Update UI Sidebar nama dan Inisial Avatar
                ...
                error: function(xhr) {
                    ...
                    if (xhr.responseJSON?.errors) {
                        msg = Object.values(xhr.responseJSON.errors)[0][0]; // Ambil pesan error spesifik dari Laravel
                    }
