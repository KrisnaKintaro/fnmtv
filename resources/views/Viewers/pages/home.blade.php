    ...
    <div class="empty-state">
      ...
      <div class="es-sub">Coba kata kunci yang berbeda</div>
        ...
        async function submitComment() {
            ...
            const beritaSlug = state.currentArticle; // Slug berita yang lagi dibuka

            // Cek ID beritanya ada apa nggak
            if (!articles[beritaSlug] || !articles[beritaSlug].id) {
                showToast('⚠️ Data berita tidak valid, coba refresh halaman');
                return;
            }
            ...
            try {
                const response = await fetch('/api/viewers/tambahKomentar', {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({
                        berita_id: articles[beritaSlug].id, // Kirim ID asli database
                        isi_komentar: text
                    })
                });
                ...
                if (response.ok) {
                    ...
                    // UI Feedback: Munculin komentar transparan (karena statusnya masih Pending)
                    renderNewPendingComment(text, 'Anda');
                } else {
                    // Kalau error (misal 401 belum login), lari ke sini
        ...
        function renderNewPendingComment(text, name) {
            const list = document.getElementById('commentsList');
            const div = document.createElement('div');
            div.className = 'comment-item';
            div.style.opacity = '0.7'; // Kasih efek agak pudar karena belum Approved
            div.innerHTML = `
        <div class="ci-avatar" style="background:var(--muted);color:#fff">${name[0].toUpperCase()}</div>
        <div class="ci-body">
            <div class="ci-user">${name} <span style="background:#eee;color:var(--muted);font-size:10px;padding:2px 6px;border-radius:8px;font-weight:700;margin-left:6px;">Menunggu Moderasi</span></div>
